...
func _run_tests() -> void:
	...
	var mine_id := agent.submit_trap_request(0, "mine", {"position": Vector2(100, 200)})
	...
	var cd_id := agent.submit_trap_request(0, "mine", {"position": Vector2(200, 200)})
	...
	_assert_eq(
		agent.submit_trap_request(1, "shotgun", {}), -1, "insufficient energy should be rejected"
	)
	...
	for i in scheduler.max_queue_size_per_team:
		var rid := agent.submit_trap_request(0, "electric_ring", ring_params)
	...
	_assert_eq(
		agent.submit_trap_request(0, "electric_ring", ring_params),
		-1,
